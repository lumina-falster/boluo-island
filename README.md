...
- **v4 新增**：每人一片独立 8×8 地块（6 人共 384 块），页签切换查看，室友地块只读，旧数据自动迁移
...
多人岛屿时顶部会出现**成员页签**：「🏝 全岛」是总览（每人一张卡片，显示各自的熟/快熟/长/空统计和下一块成熟时间）；点某个人的页签进入**TA 的专属 8×8 地块**，只能编辑自己的，看室友的是只读（详情里显示种植人）。
...
设置 → 「岛屿与成员」→ 复制**邀请链接**发给室友。室友打开链接 → 粘贴**岛屿令牌**（岛主按面板里的步骤生成一个只授权本仓库的 PAT）→ 填昵称选形象 → 加入。**每人加入后自动获得自己的一片 8×8 地块**（共 6 人），在自己的页签里种植/收获，互不干扰；「全岛」页签可看所有人的进度。换设备/退出后重新点邀请链接即可恢复身份。岛屿码（`用户名/仓库名`）也可手动在设置里填。
...
- **为什么是 8×8**：对应菠萝岛农场的 64 块地布局，每人一片。如果以后游戏改了地块数，改 `index.html` 里的 `ROWS/COLS` 常量和 `data.json` 即可（老数据打开网页会自动迁移成每人一片的格式，不用手动改）。
